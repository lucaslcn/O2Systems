 ...
 * @author Lucas
 */
public class EstadoDAO extends DAO implements IDAO_T<Estado> {
...
//        Preencha com o nome da tabela.
        ...
        if (Gema.vazio(criterio, 1)) {
            array = this.selectWithJoin(table, "nome_estado ilike '%" + criterio + "%' order by nome_estado asc");
        } else {
            array = this.select(table + " order by nome_estado asc");
        }
...
//              Definir os dados das colunas
                dadosTabela[i][0] = k.getIdestado();
                dadosTabela[i][1] = k.getUf();
                dadosTabela[i][2] = k.getNomeEstado();

                i++;
            }
        } catch (HibernateException he) {
            Mensagens.retornoAcao(
                    Mensagens.problemaPopularTabela("Estado")
...
//        580px tamanho da tabela
    ...
    public Estado consultarId(int id) {
        Object o = this.selectWithJoin("Estado", "idestado = " + id).get(0);
        return (Estado) o;
    }
